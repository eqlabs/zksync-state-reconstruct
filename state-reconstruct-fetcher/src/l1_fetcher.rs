        ...
        Ok(tokio::spawn({
            async move {
                ...
                loop {
                    ...
                    if let Some(end_block_number) = end_block {
                        match handle_main_progress(
                            &hash_tx,
                            &cancellation_token,
                        ...
                        .await
                        {
                            ProgressResult::Progressing => {}
                            ProgressResult::CaughtUp => {
                                end_block = None;
                            }
                        ...
                        end_block = handle_main_limit(metrics.clone(), &provider_clone).await;
                    }
                }
...
async fn handle_main_limit(
    metrics: Arc<Mutex<L1Metrics>>,
    provider_clone: &Provider<Http>,
) -> Option<U64> {
...
async fn handle_main_progress(
    hash_tx: &mpsc::Sender<H256>,
    cancellation_token: &CancellationToken,
    current_l1_block_number: &mut U64,
    disable_polling: bool,
    metrics: Arc<Mutex<L1Metrics>>,
    event: &Event,
    provider_clone: &Provider<Http>,
    latest_l2_block_number: &mut U256,
    previous_hash: &mut Option<H256>,
    ...
    if *current_l1_block_number > end_block_number {
        ...
        let before = Instant::now();
        if let Ok(logs) =
            L1Fetcher::retry_call(|| provider_clone.get_logs(&filter), L1FetchError::GetLogs).await
        {
            let duration = before.elapsed();
            ...
            tokio::time::sleep(Duration::from_secs(LONG_POLLING_INTERVAL_S)).await;
            return ProgressResult::Progressing;
        };
